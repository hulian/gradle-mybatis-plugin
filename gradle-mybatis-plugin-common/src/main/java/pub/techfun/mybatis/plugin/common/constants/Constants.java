package pub.techfun.mybatis.plugin.common.constants;

/**
 * @author henry
 */
public class Constants {
	public static final String GROUP_NAME = "generator";
	public static final String CONFIG_FOLDER = "generator";
	public static final String DRIVER_FOLDER = "driver";
	public static final String PROPERTY_REGISTRY_HOST = "REGISTRY_HOST";
	public static final String PROPERTY_DEPLOY_HOST = "DEPLOY_HOST";
    public static final String PROPERTY_ARGS = "ARGS";
    public static final String CONFIG_FILE = "mybatis-generator.xml";

}
